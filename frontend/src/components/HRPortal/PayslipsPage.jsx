...
// Features:
//  - Month navigation (same as PayrollPage)
//  - Search bar to filter employees
//  - Stacked payslips, one per employee
//  - Per-payslip download button + bulk download all with optional password
...
// ── Company info (edit here or pull from settings) ────────────────────────────
const COMPANY = {
  name:    "JECCA",
...
// ── Zimbabwean public holidays (same logic as PayrollPage) ────────────────────
...
function hoursForRecord(rec) {
  ...
  if (rec.status === "late") {
    ...
    return FULL_HOURS - 1; // assume 1h late if no time recorded
  }
  return 0;
}
...
// ── Single Payslip (pure rendering, also used for PDF print) ─────────────────
function Payslip({ emp, year, month, attendanceRecs, payrollRecord, edits, forPdf = false }) {
  ...
  // Build per-day breakdown from attendance records
  ...
  const border = forPdf ? "1px solid #d1d5db" : "1px solid #e2e8f0";
  const shadow = forPdf ? "none" : "0 2px 16px rgba(10,42,94,0.07)";
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
          ...
          <div>
            <div style={{
              ...
              color: "#1557b0", marginBottom: 8, paddingBottom: 6,
          ...
          <div>
            <div style={{
              fontSize: 10, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase",
              color: "#dc2626", marginBottom: 8, paddingBottom: 6,
...
// ── Password Modal ────────────────────────────────────────────────────────────
function PasswordModal({ onConfirm, onSkip, onCancel }) {
  const [pw, setPw]     = useState("");
  const [show, setShow] = useState(false);
  return (
    <div style={{
      position: "fixed", inset: 0, background: "rgba(10,42,94,0.45)",
      display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9999,
    }}>
      <div style={{
        background: "#fff", borderRadius: 16, padding: "28px 32px", width: 380,
        boxShadow: "0 20px 60px rgba(10,42,94,0.2)", fontFamily: "'DM Sans',sans-serif",
      }}>
        <div style={{ fontSize: 18, fontWeight: 700, color: "#0a2a5e", fontFamily: "'Playfair Display',serif", marginBottom: 6 }}>
          Set PDF Password
        </div>
        <div style={{ fontSize: 12.5, color: "#64748b", marginBottom: 18 }}>
          Optionally protect the downloaded PDF with a password. Leave blank to download without protection.
        </div>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          ...
          >
            {pw ? "Download Protected" : "Download"}
          </button>
          <button
            onClick={onCancel}
            style={{
              padding: "10px 16px", borderRadius: 9, border: "1.5px solid #e2e8f0",
...
// ── Print/PDF helper (browser print API) ─────────────────────────────────────
function printPayslips(htmlContent, password = "") {
  const w = window.open("", "_blank");
  if (!w) { alert("Please allow popups to download payslips."); return; }

  // Build the HTML page without embedding <script> tags inside a template
  // literal — doing so causes the browser's HTML parser to prematurely close
  // the outer <script> block on the closing </script> token.
  const pageHTML = `<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8" />
  <title>Payslips \u2014 ${new Date().toLocaleDateString()}</title>
  <link rel="preconnect" href="https://fonts.googleapis.com" />
  <link href="https://fonts.googleapis.com/css2?family=Playfair+Display:wght@600;700;800&family=DM+Sans:wght@300;400;500;600;700&display=swap" rel="stylesheet" />
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    body { font-family: 'DM Sans', sans-serif; background: #fff; color: #0f172a; }
    .payslip-wrapper { page-break-after: always; padding: 0; margin-bottom: 0; }
    .payslip-wrapper:last-child { page-break-after: avoid; }
    @media print {
...
<body>${htmlContent}</body>
  ...
  // Wait for fonts/images to load before triggering print, then close after
  // the dialog is dismissed. Using addEventListener avoids the race condition
  // where window.close() fired before the print dialog had fully opened.
  w.addEventListener("load", () => {
    if (password) {
      w.document.title = "PAYSLIP_PASSWORD:" + password;
    }
    w.focus();
    w.print();
    // Close only after the print dialog is dismissed (afterprint event),
    // with a fallback timeout for browsers that don't fire afterprint.
    const doClose = () => w.close();
    w.addEventListener("afterprint", doClose, { once: true });
    setTimeout(doClose, 60000); // safety fallback: close after 60 s
  });
}
...
export default function HRPayslipsPage({ showToast }) {
  ...
  const [showPwModal, setShowPwModal] = useState(null); // null | "all" | empId
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        if (cancelled) return;
        const [prData, attData] = await Promise.all([prRes.ok ? prRes.json():[], attRes.ok ? attRes.json():[]]);
        if (cancelled) return;
        setPayrolls(Array.isArray(prData)  ? prData  : prData.results  || []);
        setAttAll(Array.isArray(attData) ? attData : attData.results || []);
      } catch(e) { if (!cancelled) console.error("PayslipsPage:", e); }
  ...
  // Build HTML for a single employee payslip for printing
    ...
    const lateHTML = lateRecs.length > 0 ? `
      <div style="background:#fff7ed;border:1px solid #fde68a;border-radius:10px;padding:10px 14px;margin-top:12px">
        ...
        <div style="display:flex;flex-wrap:wrap;gap:6px">
          ${lateRecs.map(r => `<div style="font-size:11px;background:#fef3c7;border:1px solid #fde68a;border-radius:6px;padding:3px 8px;color:#92400e;font-weight:600">${fmtDate(r.date)}${r.arrival_time?` · ${r.arrival_time.slice(0,5)}`:""} · ${fmtHours(hoursForRecord(r))} worked</div>`).join("")}
        </div>
      </div>` : "";
...
<div class="payslip-wrapper" style="font-family:'DM Sans',sans-serif;background:#fff;overflow:hidden;border:1px solid #d1d5db">
  ...
  <div style="background:linear-gradient(135deg,#0a2a5e,#1557b0 60%,#1a6fd4);padding:22px 28px;display:flex;align-items:center;justify-content:space-between;gap:16px">
    <div style="display:flex;align-items:center;gap:14px">
      <div style="width:56px;height:56px;border-radius:12px;background:#fff;display:flex;align-items:center;justify-content:center;flex-shrink:0;overflow:hidden">
        <img src="${COMPANY.logo}" alt="${COMPANY.name}" style="width:100%;height:100%;object-fit:contain" onerror="this.style.display='none';this.parentElement.innerHTML='<span style=font-size:20px;font-weight:800;color:#0a2a5e;font-family:serif>${COMPANY.name[0]}</span>'" />
      </div>
      <div>
  ...
  <div style="padding:20px 28px">
    ...
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:16px">
      ${[["Working Days", workingDays],["Days Attended", daysAttended%1===0?daysAttended:daysAttended.toFixed(1)],["Total Hours",fmtHours(totalHours)],["Late Arrivals",lateRecs.length]].map(([l,v])=>`
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:16px">
      ...
      <div>
        ...
        ${deduction>0?`<div style="margin-top:8px;padding:8px 0;border-top:2px solid #dc2626;display:flex;justify-content:space-between"><span style="font-size:12px;font-weight:700;color:#dc2626">Total Deductions</span><span style="font-size:14px;font-weight:800;color:#dc2626;font-family:monospace">-${fmtUSD(deduction)}</span></div>`:""}
      </div>
    </div>
    ${lateHTML}
    <!-- Net Pay -->
  ...
  const handleDownloadOne = (emp) => { setShowPwModal(emp.id); };
  const handleDownloadAll = () => { setShowPwModal("all"); };

  const executePrint = useCallback((targetId, password) => {
    setShowPwModal(null);
    if (targetId === "all") {
      const html = filtered.map(emp => buildPayslipHTML(emp)).join("\n");
      printPayslips(html, password);
    } else {
      const emp = filtered.find(e => e.id === targetId);
      if (emp) printPayslips(buildPayslipHTML(emp), password);
    }
  }, [filtered, buildPayslipHTML]);
  ...
  return (
    <>
      <style>{`
        ...
        .payslip-page-btn:hover { opacity: .85; }
      `}</style>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
          ...
          <div style={{ display:"flex", gap:10, alignItems:"center", flexWrap:"wrap" }}>
            ...
            <div style={{ display:"flex", alignItems:"center", gap:4, background:"#fff", border:"1.5px solid #e2e8f0", borderRadius:10, padding:"4px 6px" }}>
              ...
              <button onClick={nextMonth} disabled={isCurrentMonth} style={{ background:"none", border:"none", cursor: isCurrentMonth?"not-allowed":"pointer", color: isCurrentMonth?"#cbd5e1":"#475569", padding:"4px 8px", borderRadius:7 }}>
            ...
            <div style={{ position:"relative" }}>
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#94a3b8" strokeWidth="2.2" strokeLinecap="round" style={{ position:"absolute", left:10, top:"50%", transform:"translateY(-50%)", pointerEvents:"none" }}>
            ...
            <button
              ...
              style={{
                ...
                fontFamily:"'DM Sans',sans-serif", cursor: loading||filtered.length===0 ? "not-allowed":"pointer",
        ...
        {!loading && filtered.map((emp, idx) => {
          ...
          return (
            <div key={emp.id} style={{ display:"flex", flexDirection:"column", gap:0 }}>
              {/* Per-payslip download button row */}
              <div style={{
                display:"flex", alignItems:"center", justifyContent:"space-between",
      ...
      {showPwModal && (
        <PasswordModal
          onConfirm={(pw) => executePrint(showPwModal, pw)}
          onCancel={()  => setShowPwModal(null)}
        />
      )}
    </>
  );
