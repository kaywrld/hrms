...
function buildSessions(activity) {
  const sessions = [];
  if (!activity || !activity.length) return sessions;
...
function downloadSessionReport(session, records, adminName) {
  const loginDt  = new Date(session.login.timestamp);
  const logoutDt = session.logout ? new Date(session.logout.timestamp) : null;
  const fmt      = d => d.toLocaleString("en-ZW", { dateStyle: "long", timeStyle: "short" });
  ...
  const rows = records.map((r, i) => {
    const s = statusMeta[r.status] || { bg: "#f1f5f9", color: "#64748b" };
    const label = r.status.charAt(0).toUpperCase() + r.status.slice(1).replace("_", " ");
    return `<tr style="background:${i % 2 === 0 ? "#fff" : "#f8faff"}">
      <td style="padding:9px 14px;border-bottom:1px solid #e8edf6;font-size:13px;font-weight:500;color:#0f172a">${r.employee_name || "—"}</td>
      <td style="padding:9px 14px;border-bottom:1px solid #e8edf6;font-size:13px;color:#64748b">${r.date}</td>
      <td style="padding:9px 14px;border-bottom:1px solid #e8edf6">
        <span style="padding:2px 12px;border-radius:20px;font-size:11px;font-weight:700;background:${s.bg};color:${s.color}">${label}</span>
      </td>
    </tr>`;
  }).join("");
...
<style>
  ...
  .header{background:linear-gradient(135deg,#0a2a5e,#1557b0);padding:36px 48px;color:#fff}
  .header h1{font-size:24px;font-weight:800;margin:0 0 4px}
  .header p{font-size:13px;color:rgba(255,255,255,0.65);margin:0}
  .meta{display:flex;gap:28px;margin-top:22px}
  ...
  .stat{flex:1;border-radius:10px;padding:14px;border:1px solid #e2e8f0;text-align:center}
  .stat .n{font-size:26px;font-weight:800;color:#0a2a5e;line-height:1}
  .stat .l{font-size:11px;color:#64748b;margin-top:3px}
  table{width:100%;border-collapse:collapse}
  thead tr{background:#0a2a5e}
  thead th{padding:10px 14px;text-align:left;font-size:10px;font-weight:700;color:#fff;text-transform:uppercase;letter-spacing:0.7px}
  .foot{margin-top:28px;padding-top:14px;border-top:1px solid #e2e8f0;display:flex;justify-content:space-between;font-size:11px;color:#94a3b8}
...
<div class="header">
  ...
  <div class="meta">
    ...
    <div class="meta-item"><label>Login Time</label><span>${fmt(loginDt)}</span></div>
    <div class="meta-item"><label>Logout Time</label><span>${logoutDt ? fmt(logoutDt) : "Still Active"}</span></div>
  </div>
</div>
<div class="body">
  <div class="stats">
    <div class="stat"><div class="n">${records.length}</div><div class="l">Total Marked</div></div>
    <div class="stat" style="border-color:#bbf7d0"><div class="n" style="color:#059669">${present}</div><div class="l">Present</div></div>
    <div class="stat" style="border-color:#fecaca"><div class="n" style="color:#dc2626">${absent}</div><div class="l">Absent</div></div>
    <div class="stat" style="border-color:#fde68a"><div class="n" style="color:#d97706">${late}</div><div class="l">Late</div></div>
  </div>
  <table>
    <thead><tr><th>Employee Name</th><th>Date</th><th>Status</th></tr></thead>
    <tbody>${rows || `<tr><td colspan="3" style="padding:24px;text-align:center;color:#94a3b8;font-size:13px">No attendance records for this session.</td></tr>`}</tbody>
  </table>
  <div class="foot"><span>HRMS Attendance Report</span><span>Generated: ${new Date().toLocaleString("en-ZW")}</span></div>
</div>
</body></html>`;
  ...
  a.href     = url;
  a.download = `attendance_${adminName.replace(/\s+/g, "_")}_${loginDt.toISOString().slice(0, 10)}.html`;
  a.click();
...
function SessionPanel({ session, adminId, adminName, onBack, showToast }) {
  ...
  const handleDownload = () => {
    if (!records) return;
    setDlBusy(true);
    try { downloadSessionReport(session, records, adminName); }
    finally { setTimeout(() => setDlBusy(false), 600); }
  };
    ...
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
        <button onClick={onBack} style={{
          ...
          color: "#1557b0", border: "none", borderRadius: 8, padding: "6px 12px",
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4 }}>
              ...
              <span style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: 0.7, color: "#64748b" }}>Login</span>
            </div>
            <div style={{ fontSize: 13.5, fontWeight: 700, color: "#0f172a" }}>{fmtDT(session.login.timestamp)}</div>
          </div>
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4 }}>
              <div style={{ width: 8, height: 8, borderRadius: "50%", background: session.logout ? "#dc2626" : "#94a3b8" }} />
              <span style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: 0.7, color: "#64748b" }}>Logout</span>
            </div>
            {session.logout
              ? <div style={{ fontSize: 13.5, fontWeight: 700, color: "#0f172a" }}>{fmtDT(session.logout.timestamp)}</div>
              : <span style={{ display: "inline-flex", alignItems: "center", gap: 5, background: "#ecfdf5", color: "#059669", borderRadius: 20, padding: "3px 10px", fontSize: 11, fontWeight: 700 }}>
                  ● Currently Active
                </span>
            }
          </div>
        ...
        <>
          <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
            {[
              ...
              <div key={lbl} style={{
                flex: 1, background: bg, borderRadius: 10, padding: "10px 8px",
                textAlign: "center", border: `1px solid ${color}22`,
              }}>
                <div style={{ fontSize: 20, fontWeight: 800, color, lineHeight: 1 }}>{val}</div>
                <div style={{ fontSize: 9, color, fontWeight: 700, marginTop: 3, textTransform: "uppercase", letterSpacing: 0.5 }}>{lbl}</div>
              </div>
            ))}
          </div>
            ...
            {records.map((r, i) => {
              ...
              return (
                <div key={r.id} style={{
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontWeight: 600, fontSize: 13, color: "#0f172a", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{r.employee_name}</div>
                    <div style={{ fontSize: 11, color: "#94a3b8" }}>{r.date}</div>
                  </div>
                  <span style={{ padding: "2px 10px", borderRadius: 20, fontSize: 11, fontWeight: 700, background: st.bg, color: st.color, whiteSpace: "nowrap" }}>
                    {label}
                  </span>
                </div>
              );
      ...
      <button onClick={handleDownload} disabled={!records || dlBusy} style={{
        ...
        background: "linear-gradient(135deg,#0a2a5e,#1557b0)", color: "#fff",
        cursor: (!records || dlBusy) ? "not-allowed" : "pointer",
        opacity: (!records || dlBusy) ? 0.6 : 1,
        ...
        {dlBusy ? "Preparing download…" : `Download Report (${records ? records.length : 0} records)`}
      </button>
    </div>
  );
}
        ...
        <div style={{
          ...
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 16 }}>
            ...
            <button onClick={() => onEdit(localAdmin)} style={{
              ...
              border: "1px solid rgba(255,255,255,0.3)", borderRadius: 9, cursor: "pointer",
              color: "#fff", fontFamily: "'DM Sans',sans-serif", fontSize: 12.5, fontWeight: 600,
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 16 }}>
            ...
            <div>
              <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 19, fontWeight: 700, color: "#fff" }}>
                {localAdmin.full_name}
              </div>
              <div style={{ fontSize: 12, color: "rgba(255,255,255,0.6)", marginTop: 2 }}>@{localAdmin.username}</div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 16 }}>
            {[
              ...
              <div key={label} style={{ background: "rgba(255,255,255,0.1)", borderRadius: 10, padding: "8px 12px" }}>
                <div style={{ fontSize: 9, fontWeight: 700, textTransform: "uppercase", letterSpacing: 1, color: "rgba(255,255,255,0.45)", marginBottom: 2 }}>{label}</div>
                <div style={{ fontSize: 12, fontWeight: 600, color: "#fff", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{val}</div>
              </div>
            ))}
          </div>
          ...
          {tab === "info" && (
            <>
              <div style={{
                ...
                <div>
                  ...
                  <div style={{ fontSize: 12, color: "#64748b", marginTop: 2 }}>
                    {localAdmin.is_active ? "Block this admin from logging in." : "Restore login access for this admin."}
                  </div>
                </div>
              ...
              <div style={{ marginBottom: 20 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                  {[
                    ...
                    ["Status",       localAdmin.is_active ? "Active" : "Inactive"],
                    ["Member Since", localAdmin.created_at
                      ? new Date(localAdmin.created_at).toLocaleDateString("en-ZW", { dateStyle: "medium" })
                      : "—"],
                  ].map(([label, val]) => (
                    <div key={label}>
                      <div style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: 0.7, color: "#94a3b8", marginBottom: 3 }}>{label}</div>
                      <div style={{ fontSize: 13.5, color: "#0f172a", fontWeight: 600 }}>{val || "—"}</div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div>
                <div style={{
                  ...
                  {sessions.length > 0 && (
                    <button onClick={() => setTab("history")} style={{
                      background: "#eff6ff", color: "#1557b0", border: "none",
                ...
                {activity === null ? (
                  <div style={{ color: "#94a3b8", fontSize: 13, textAlign: "center", padding: "18px 0" }}>Loading activity…</div>
                ) : sessions.length === 0 ? (
                  <div style={{ color: "#94a3b8", fontSize: 13, textAlign: "center", padding: "18px 0" }}>No login activity recorded yet.</div>
                ) : (() => {
                  const s = sessions[0];
                  ...
                  return (
                    <div style={{
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                        ...
                        <div>
                          <div style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: 0.6, color: "#64748b" }}>Login Time</div>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                        ...
                        <div>
                          <div style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: 0.6, color: "#64748b" }}>Logout Time</div>
                          {isActive
                            ? <span style={{ display: "inline-flex", alignItems: "center", gap: 5, background: "#ecfdf5", color: "#059669", borderRadius: 20, padding: "2px 10px", fontSize: 11, fontWeight: 700, marginTop: 1 }}>
                                ● Currently Online
                              </span>
                            : <div style={{ fontSize: 14, fontWeight: 700, color: "#0f172a", marginTop: 1 }}>{fmtDT(s.logout.timestamp)}</div>
                          }
                        </div>
                      </div>
            ...
            sessionView ? (
              ...
              <div>
                <div style={{
                  ...
                  <span>All Login Sessions</span>
                  <span style={{ fontSize: 11, color: "#94a3b8", fontWeight: 500, textTransform: "none", letterSpacing: 0 }}>
                    {activity === null ? "…" : `${sessions.length} session${sessions.length !== 1 ? "s" : ""}`}
                  </span>
                </div>
                        ...
                        >
                          <div style={{ display: "flex", justifyContent: "space-between", gap: 10 }}>
                            <div style={{ flex: 1 }}>
                              ...
                              <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
                                ...
                                {isActive
                                  ...
                                  : <span style={{ fontSize: 13, fontWeight: 600, color: "#0f172a" }}>{fmtDT(s.logout.timestamp)}</span>
                                }
                              </div>
                              {s.login.ip_address && (
                                <div style={{ fontSize: 10, color: "#94a3b8", marginTop: 5 }}>IP: {s.login.ip_address}</div>
                              )}
                            </div>
                            <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", justifyContent: "space-between", flexShrink: 0 }}>
                              ...
                              <span style={{ fontSize: 11, color: "#1557b0", fontWeight: 600, display: "flex", alignItems: "center", gap: 3, marginTop: 8 }}>
                                View Attendance
